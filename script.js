...
const MAX_CATCHUP_SECONDS = 3 * 60 * 60;
...
let state = {
  ...
  strawberries: 0,
  coins: 0,
  toys: { piglet: 0, cowboy: 0, plush: 0, ball: 0, kite: 0, yoyo: 0, blocks: 0 },
  clothes: { bow: 0, scarf: 0, glasses: 0, hat: 0, cap: 0, flower: 0 },
  equipped: { neck: null, eyes: null, head: null, ear: null },
  carePoints: 0,
  isDead: false,
  isTorn: false,
  isSick: false,
...
function renderDirt() {
  const dirtiness = clamp(100 - state.clean);
...
function finishWork() {
  ...
  const earned = Math.floor(10 + Math.random() * 21);
  state.coins += earned;
...
const SPECIAL_ITEM_DURATION = 20 * 60 * 1000;
const SPECIAL_ITEMS = [
  { id: "weichspueler", emoji: "🧴✨", name: "Weichspüler", desc: "Hält Summi 20 Minuten lang länger sauber", cost: 35, currency: "coins", effect: "clean" },
  { id: "erdbeerkuchen", emoji: "🍓🍰", name: "Erdbeerkuchen", desc: "Hält Summi 20 Minuten lang länger satt", cost: 30, currency: "coins", effect: "hunger" },
  { id: "energydrink", emoji: "⚡🥤", name: "Energy-Drink", desc: "Hält Summi 15 Minuten lang länger wach", cost: 25, currency: "coins", effect: "wake" },
  { id: "vitamintropfen", emoji: "💧💪", name: "Vitamintropfen", desc: "Schützt 30 Minuten lang vor Krankheit", cost: 40, currency: "coins", effect: "sickImmune" },
  { id: "gluecksklee", emoji: "🍀", name: "Glücksklee", desc: "Das nächste Booster-Pack enthält garantiert eine seltene Karte", cost: 15, currency: "strawberries", effect: "luckyClover" },
];

const CLOTHES_PREVIEW = {
  bow: { viewBox: "115 165 70 42", inner: `
  ...
  scarf: { viewBox: "90 172 120 68", inner: `
  ...
  glasses: { viewBox: "85 88 130 48", inner: `
  ...
  hat: { viewBox: "95 10 110 70", inner: `
  ...
  cap: { viewBox: "95 28 130 62", inner: `
  ...
  flower: { viewBox: "61 34 34 30", inner: `
...
function startToyPlay(id) {
  ...
  void el.toyPlayFill.offsetWidth;
...
const BOOSTER_PACKS = [
  { id: "basic", name: "Kuschelmomente", subtitle: "Standard-Pack", desc: "3 zufällige Karten", cost: 40, currency: "coins", cardCount: 3, boosted: false, coverImg: "pack-cover.jpg", gradient: "linear-gradient(160deg,#7FB3B8,#5FA3A8)" },
  { id: "premium", name: "Sternenstaub", subtitle: "Glücks-Pack", desc: "3 Karten, bessere Chance auf Seltenes!", cost: 12, currency: "strawberries", cardCount: 3, boosted: true, coverImg: "pack-cover.jpg", gradient: "linear-gradient(160deg,#C79AF0,#8F5FD1)" },
  { id: "adventure", name: "Abenteuer-Zeit", subtitle: "Abenteuer-Pack", desc: "3 zufällige Karten", cost: 55, currency: "coins", cardCount: 3, boosted: false, coverImg: "pack-cover.jpg", gradient: "linear-gradient(160deg,#A9CC8C,#6E9A56)" },
  { id: "sunny", name: "Strand & Sonne", subtitle: "Sommer-Pack", desc: "3 zufällige Karten", cost: 60, currency: "coins", cardCount: 3, boosted: false, coverImg: "pack-cover.jpg", gradient: "linear-gradient(160deg,#FFD97D,#F2A65A)" },
  { id: "kitchen", name: "Kuschel-Küche", subtitle: "Leckerbissen-Pack", desc: "3 zufällige Karten", cost: 45, currency: "coins", cardCount: 3, boosted: false, coverImg: "pack-cover.jpg", gradient: "linear-gradient(160deg,#F4A6A6,#E2564F)" },
  { id: "dream", name: "Traumreise", subtitle: "Nacht-Pack", desc: "3 Karten, bessere Chance auf Seltenes!", cost: 20, currency: "strawberries", cardCount: 3, boosted: true, coverImg: "pack-cover.jpg", gradient: "linear-gradient(160deg,#7B8FD1,#4A5A9E)" },
];
...
function openBoosterPack(packId) {
  ...
  const RARITY_RANK = { common: 0, rare: 1, ultra: 2, legendary: 3 };
  if (state.effects.luckyClover && !drawnCards.some((c) => RARITY_RANK[c.rarity] > 0)) {
    const upgraded = pickRandomCard("rare");
    if (upgraded) drawnCards[0] = upgraded;
    state.effects.luckyClover = false;
    showToast("🍀 Der Glücksklee hat gewirkt!");
  } else if (state.effects.luckyClover) {
    state.effects.luckyClover = false;
  }
...
function startGame(key) {
  ...
  overlayPanel.classList.add("no-scroll");
...
const BALLOON_DURATION = 22;
const BALLOON_RISE_MIN = 3.4;
...
function updateBalloonPositions(dt) {
  ...
  for (const b of balloonState.balloons) {
    ...
    b.elapsed += dt;
    b.topPct = 108 - (b.elapsed / b.riseDuration) * 128;
